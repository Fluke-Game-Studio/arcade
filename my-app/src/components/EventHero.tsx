...
type EventHeroProps = {
  ...
  showMuteMotion?: boolean;
  storageKey?: string;
  events?: EventHeroEvent[];
...
function resolveEventEndAt(event: {
  ...
  const mins = Number.isFinite(event.durationMinutes)
    ? Math.max(1, Number(event.durationMinutes))
    : 60;
  return new Date(event.at.getTime() + mins * 60_000);
}
...
export default function EventHero({
  ...
  const [muteMotion, setMuteMotion] = useState<boolean>(() => {
    try {
      return localStorage.getItem(`${storageKey}:muteMotion`) === "1";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      if (isControlled) {
        const next = (activeIndex + 1) % normalizedEvents.length;
        onEventChange?.(normalizedEvents[next].id);
        return;
      }

      setInternalIndex((prev) => {
        const next = (prev + 1) % normalizedEvents.length;
        onEventChange?.(normalizedEvents[next].id);
        return next;
      });
    }, autoRotateMs);
  ...
  const { d, h, m, s, diffMs, nowMs } = useCountdown(active.at);
  ...
  const resolvedStatus: "SCHEDULED" | "LIVE" | "ENDED" = useMemo(() => {
    ...
    const now = Date.now();
    if (now >= activeEndAt.getTime()) return "ENDED";
    if (now >= active.at.getTime()) return "LIVE";
    return "SCHEDULED";
  }, [active.status, active.at, activeEndAt]);
  ...
  const progress = useMemo(() => {
    ...
    return clamp01((nowMs - startWindow) / windowMs);
  }, [active.at, nowMs]);
  ...
  const reminderState = useMemo(() => {
    ...
    if (diffMs <= 10 * 60 * 1000) return "NOW" as const;
    if (diffMs <= 60 * 60 * 1000) return "SOON" as const;
    return "ON" as const;
          ...
          >
            <div style={{ minWidth: 0 }}>
              ...
              >
                ...
                <MetaChip icon="place" text={active.location || "Online"} />
                <MetaChip icon="groups" text={active.host || "Leadership Team"} />
              </div>
